...
# - Modifies /var/www/html/supermon/link.php to include custom/announcement.inc (idempotent)
...
# 6. Modify link.php to include custom/announcement.inc (idempotent)
echo_step "6. Modifying $LINK_PHP to include custom/announcement.inc"

if [[ ! -f "$LINK_PHP" ]]; then
    warn "$LINK_PHP not found – skipping modification"
else
    # Check if we already have the announcement include
    if grep -q 'include_once\s\+["'"'"']custom/announcement\.inc["'"'"']' "$LINK_PHP"; then
        echo "Announcement include already present in $LINK_PHP – no changes needed"
    else
        echo "Adding include_once \"custom/announcement.inc\"; before footer include..."

        # Insert just before any variation of: include "footer.inc";  or include_once 'footer.inc';
        # This pattern is quite forgiving
        if sed -i '/include.*["'"'"']footer\.inc["'"'"'];/i \
include_once "custom/announcement.inc";' "$LINK_PHP"; then
            echo "Successfully added the announcement include to $LINK_PHP"
        else
            warn "sed command failed – you may need to manually edit $LINK_PHP"
            warn "Desired ending:"
            warn '<?php'
            warn 'include_once "custom/announcement.inc";'
            warn 'include_once "footer.inc";'
            warn '?>'
        fi
    fi
